...
class PeriodicFit(BaseFit):
    ...
    def trigXrow(self, theta, ndiff, pdiff, symnumber, order):
        ...
        theta       -- Input angle
        ndiff       -- Number of derivates on basis
        pdiff       -- A specific smoothing order
                       (corresponding to the overall fitting function)
        symnumber   -- Divide 2pi complete rotation with this number
        ...
        for nthbase in range(2, order, 2):
            ...
            coeff = k**coeffdiff
            xbasisval[nthbase-1] = coeff*basefuncs[0](k*theta)
            xbasisval[nthbase] = coeff*basefuncs[1](k*theta)
        ...
        if order % 2 == 0:
            k = symnumber*(order/2)
            coeff = k**coeffratio
            xbasisval[order-1] = coeff*basefuncs[0](k*theta)
        return xbasisval
